...
class QueuedValidation < ApplicationRecord
    ...
    def queue
      UnqueueValidationJob.queue(queue_name, :id => self.id)
    end
    ...
    def allocate_ip_address
      if Postal.ip_pools? && self.validation && pool = self.server.ip_pool_for_message(self.validation)
    ...
    def batchable_validations(limit = 10)
      unless locked?
        raise Postal::Error, "Must lock current validation before locking any friends"
      end
      if self.batch_key.nil?
        ...
        QueuedValidation.where(:batch_key => self.batch_key, :ip_address_id => self.ip_address_id, :locked_by => locker, :locked_at => time)
      end
    end
