            ...
            ([original_validation] + other_validations).each do |queued_validation|
                ...
                begin
                    ...
                    if queued_validation.attempts >= Postal.config.general.maximum_delivery_attempts
                        ...
                        if queued_validation.server.validation_db.suppression_list.add(:recipient, queued_validation.validation.rcpt_to, :reason => "too many soft fails")
                    ...
                    # If the raw validation has been removed (removed by retention)
                    unless queued_validation.validation.raw_validation?
                        log "#{log_prefix} Raw validation has been removed. Not sending."
                        queued_validation.validation.create_delivery('HardFail', :details => "Raw validation has been removed. Cannot send validation.")
                        queued_validation.destroy
                        next
                    end
                    ...
                    #
                    if !queued_validation.manual? && sl = queued_validation.server.validation_db.suppression_list.get(:recipient, queued_validation.validation.rcpt_to)
                    ...
                    # Update the live stats for this validation.
                    queued_validation.validation.database.live_stats.increment(queued_validation.validation.scope)
                    ...
                    begin
                        if @fixed_result
                            ...
                            sender = cached_sender(Postal::SMTPSender, queued_validation.validation.recipient_domain, queued_validation.ip_address)
                    ...
                    if result.type == 'HardFail'
                        recent_hard_fails = queued_validation.server.validation_db.select(:validations, :where => {:rcpt_to => queued_validation.validation.rcpt_to, :status => 'HardFail', :timestamp => {:greater_than => 24.hours.ago.to_f}}, :count => true)
                        if recent_hard_fails >= 1
                            if queued_validation.server.validation_db.suppression_list.add(:recipient, queued_validation.validation.rcpt_to, :reason => "too many hard fails")
                    ...
                    #
                    if result.type == 'Sent'
                        if queued_validation.server.validation_db.suppression_list.remove(:recipient, queued_validation.validation.rcpt_to)
